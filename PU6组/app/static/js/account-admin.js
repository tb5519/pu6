...
const accountTeacherSelect = document.querySelector("#account-teacher");
...
function renderAccountTeacherOptions() {
  if (!accountTeacherSelect) return;
  const currentValue = accountTeacherSelect.value;
  accountTeacherSelect.innerHTML = `
    <option value="">选择班主任</option>
    ${accountTeachers.map((teacher) => `
      <option value="${escapeAccountText(teacher.id)}"${teacher.id === currentValue ? " selected" : ""}>
        ${escapeAccountText(teacher.name)}
      </option>
    `).join("")}
  `;
}
...
function renderAccounts() {
  ...
  if (!accountList.length) {
    accountRows.innerHTML = `<tr><td colspan="5" class="database-empty-cell">暂无账号。</td></tr>`;
    return;
  }
...
async function loadAccounts() {
  ...
  accountTeachers = data.teachers || [];
  accountList = data.accounts || [];
  renderAccountTeacherOptions();
  renderAccounts();
  setAccountMessage("");
}
...
async function createAccount(event) {
  ...
  const teacherId = accountTeacherSelect?.value || "";
  const password = accountPasswordInput?.value || "";

  if (!username || !teacherId || !password) {
    setAccountMessage("请填写账号、对应班主任和初始密码。", true);
    return;
  }
